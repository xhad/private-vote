...
DB_USER="root"          
DB_PASSWORD="pwd"      
SQL_FILE="./modules/snapshot-hub/src/helpers/schema.sql"

# Copy the SQL file to the container
docker cp "$SQL_FILE" "$CONTAINER_NAME":/home/node/app/schema.sql

# Execute the SQL file in the container
docker exec -i "$CONTAINER_NAME" /bin/bash -c "mysql -h snapshot-mysql -u $DB_USER -p$DB_PASSWORD $DB_NAME < schema.sql"
